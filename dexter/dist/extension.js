...
class Codex {
    ...
    async complete() {
        ...
        this.queries.forEach(async (query) => {
            ...
            requests.push(axios_1.default.post('https://api.openai.com/v1/engines/davinci-codex/completions', {
                method: "POST",
                headers: headers,
...
const codex_1 = __webpack_require__(2);
// this method is called when your extension is activated
// your extension is activated the very first time the command is executed
function activate(context) {
    let codex = new codex_1.Codex();
    ...
    function appendTextAfterLine(text, selection) {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
    ...
    function replaceTextAtSelection(text, selection) {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            editor.edit(edit => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dexter.showCodexContext', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dexter.runCodeCompletion', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dexter.insertCodeCompletion', async () => {
        const key = vscode.workspace.getConfiguration('dexter').get('apiKey', false);
        if (!key) {
            await vscode.window.showInputBox({
                title: "Set the OpenAI API key"
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dexter.setCodexResLength', async () => {
        await vscode.window.showInputBox({
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dexter.setStopSequences', async () => {
        await vscode.window.showInputBox({
